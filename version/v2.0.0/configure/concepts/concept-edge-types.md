...
| Discrete Graph Type | <p>The type of relationship between two concept classes. There is currently only one option.<br>- Taxonomy <br><br>A taxonomy concept edge type is fundamentally a directed acyclic graph (DAG). Defining a concept edge type's discrete graph type as taxonomy specifies that the relationship between the two concept classes are modeled as a DAG of taxonomic data.</p> |
